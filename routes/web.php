...
Route::group(['middleware' => ['auth', 'roles'],'roles' => 'admin','prefix'=>'admin'], function () {
    ...
    //Route For Change Coupon Status
    Route::get('change-coupon-status','Admin\CouponController@changeStatus')->name('changeCouponStatus');
    ...
    Route::get('changePrdStatus', 'Admin\AdminController@changePrdStatus')->name('changePrdStatus');
    ...
    Route::get('product/tags', 'Admin\\ProductController@productTags')->name('product.tags');
    
    /*Route For Sale Change Status*/
    Route::get('changestatus','Admin\\ProductController@changeStatus')->name('changestatus');
    
    /*Route For New Product Date*/
    Route::get('change-new-date','Admin\\ProductController@changeNewDate')->name('changeNewDate');
    
    /*Route For New Product Date*/
    Route::get('change-backorder','Admin\\ProductController@changeBackOrder')->name('changeBackOrder');
    
    // Route For Banner Change Status
    Route::get('banner-changestatus','Admin\\BannerController@changeStatus')->name('bannerChangestatus');
    ...
    //Route for Update product status
    Route::post('update-attribute-order','Admin\\AttributesController@updateOrder')->name('update-attribute-order');
...
// Route::get('/new-for-2023','HomeController@newFor')->name('new.for');
Route::get('/custom-orders-kims-gold-dust-custom-corvette-jewelry','HomeController@cutsomOrders')->name('custom.orders');
Route::get('/testimonial','HomeController@testimonial')->name('testimonial');
// Route::post('password/reset', 'Auth\ForgotPasswordController@sendPasswordResetToken')->name('password.email');
...
//Reset Password
Route::get('/our-story-kims-gold-dust-corvette-jewelry-corvette-accessories','HomeController@ourStory')->name('our.story');
Route::get('/the-firing-process','HomeController@firingProcess')->name('firing.process');

Route::get('/kims-gold-dust-show-schedule','HomeController@schedule')->name('schedule');
Route::get('contact','HomeController@contact')->name('contact');
Route::get('ordering-information-faq','HomeController@faq')->name('faq');
Route::get('privacy-policy','HomeController@privacyPolicy')->name('privacy.policy');
Route::get('helpful-tips','HomeController@helpfulTips')->name('helpful.tips');
Route::get('jewelry-care','HomeController@jewelryCare')->name('jewelry.care');
...
Route::post('get-shipping-tax','HomeController@shippingTax')->name('shippingTax');
Route::post('get/product/details', 'HomeController@getProductDetails')->name('getProductDetails');
Route::post('product-checkout','OrderController@prdCheckout')->name('prdCheckout');
Route::post('check/tax', 'HomeController@checkTax')->name('check.tax');
...
Route::get('product/{slug}','ProductController@shopDetail')->name('shopDetail');
...
/*Save For Later*/
Route::get('/save-for-later/{id}/{cartId}', 'SaveLaterController@index')->name('customer.save.product');
Route::get('/delete-save-product/{id}', 'SaveLaterController@delete')->name('remove.save.product');

Route::get('save-prod-cart/{id}','SaveLaterController@savePrdCart')->name('savePrdCart');
...
Route::get('{slug?}','ProductController@categoryDetail')->name('categoryDetail');
...
Route::resource('admin/redirection', 'Admin\\RedirectionController');
...
Route::resource('product-videos/product-videos', 'Admin\ProductVideosController');
Route::resource('story/story', 'Admin\StoryController');
Route::resource('schedule/schedule', 'Admin\ScheduleController');
Route::post('schedule-order/order', 'Admin\ScheduleController@scheduleOrder')->name('schedule.order');
Route::post('banner-order/order', 'Admin\BannerController@bannerOrder')->name('banner.order');
Route::resource('firingprocessslider/firing-process-slider', 'Admin\FiringProcessSliderController');
Route::resource('coupon/coupon', 'Admin\CouponController');
Route::resource('firingprocessvideo/firing-process-video', 'Admin\FiringProcessVideoController');
Route::resource('shippings/shippings', 'shippings\ShippingsController');
Route::post('admin/delete-tax-variant', 'shippings\shippingscontroller@deleteprovariant')->name('delete.tax.variant');
Route::resource('thankyou/thankyou', 'thankyou\ThankyouController');
Route::resource('orderemail/orderemail', 'orderemail\OrderemailController');
Route::resource('newsletteremail/newsletteremail', 'newsletteremail\NewsletteremailController');
Route::resource('discount/discount', 'discount\DiscountController');
Route::resource('newaccount/newaccount', 'newaccount\NewaccountController');